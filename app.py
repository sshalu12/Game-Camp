...
def flappy():
  ...
  if __name__ == "__main__":
    ...
    GAME_SPRITES['background'] = pygame.image.load('background.jpeg').convert()
    GAME_SPRITES['player'] = pygame.image.load('bird.png').convert_alpha()
...
def snake():
  ...
  screen = Screen()
  user_input = screen.textinput(title="PONG GAME", prompt="ENTER Y FOR START THE GAME")
  game_start = False
...
def car_crossing():
  ...
  screen.setup(width=600, height=600)
  screen.tracer(0)
  user_input = screen.textinput(title="CAR CROSSING GAME", prompt="ENTER Y FOR START THE GAME")
  game_start = False
